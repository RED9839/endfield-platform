...
function WeaponCard({ weapon }: { weapon: WeaponLike }) {
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full p-2.5 sm:p-3">
        ...
        <div className="mt-2 flex h-[18px] flex-nowrap gap-1 overflow-hidden">
          ...
          {weaponSeriesText ? (
            <WeaponChip muted>{weaponSeriesText}</WeaponChip>
          ) : null}
        </div>
      </div>
    </Link>
...
export default function WeaponsPage() {
  ...
  const filteredWeapons = useMemo(() => {
    ...
    return (weaponDetails as WeaponLike[]).filter((weapon) => {
      ...
      const matchesKeyword =
        ...
        getWeaponTypeLabel(weapon).toLowerCase().includes(normalizedKeyword) ||
        rawSeriesText.toLowerCase().includes(normalizedKeyword) ||
  ...
  const sortedWeapons = useMemo(() => {
    return [...filteredWeapons].sort((leftWeapon, rightWeapon) => {
      const leftRarity = getWeaponRarity(leftWeapon);
      const rightRarity = getWeaponRarity(rightWeapon);

      if (rightRarity !== leftRarity) return rightRarity - leftRarity;

      const leftTypeOrder = weaponTypeOrderMap[getWeaponType(leftWeapon)] ?? 999;
      const rightTypeOrder =
        weaponTypeOrderMap[getWeaponType(rightWeapon)] ?? 999;

      if (leftTypeOrder !== rightTypeOrder) {
        return leftTypeOrder - rightTypeOrder;
      }

      return leftWeapon.name.localeCompare(rightWeapon.name, "ko");
    });
  }, [filteredWeapons]);
