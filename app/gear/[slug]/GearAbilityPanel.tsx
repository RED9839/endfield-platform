...
function highlightElementTerms(text: string): ReactNode {
  const colorMap: Array<{ pattern: RegExp; color: string }> = [
    ...
    { pattern: /힘/g, color: YELLOW_MAIN },
    { pattern: /민첩/g, color: YELLOW_MAIN },
    { pattern: /지능/g, color: YELLOW_MAIN },
    { pattern: /의지/g, color: YELLOW_MAIN },
  ];
...
export default function GearAbilityPanel({
  title,
  block,
}: {
  title: string;
  ...
  useEffect(() => {
    const isValid = upgradeButtonMap.some(
      (item) => item.key === selectedUpgrade
    );
    if (!isValid) {
