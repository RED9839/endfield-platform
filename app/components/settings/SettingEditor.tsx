...
function applyStatByLabel(
  ...
  if (normalizedLabel.includes("냉기") && normalizedLabel.includes("전기")) {
    ...
    normalizedLabel.includes("냉기 피해") ||
    normalizedLabel.includes("전기 피해") ||
    normalizedLabel.includes("열기 피해") ||
    normalizedLabel.includes("자연 피해")
  ) {
    if (allowDynamicStats) addDynamicStat(target, normalizedLabel, value, isPercent);
    handled = true;
  }
...
function getOperatorBreakthroughBonusStats(
  ...
  const items = getOperatorBreakthroughItems(operator);
  if (items.length <= 0 || activeRank < 2) return target;

  const secondStage =
    items.find((item, index) => getStageNumber(item, index) === 2)?.item ??
    items[1] ??
    null;

  if (secondStage) {
    applyBreakthroughStageStats(target, secondStage, {
      allowDynamicStats: true,
    });
  }
...
function FinalStatPanel({
  ...
  function getPercentDetailRows(
    ...
    const rows: { label: string; value: string }[] = [];
    const sources = [
      { label: "장비 속성", stat: Number(breakdown.gear[key] ?? 0) },
  ...
  function getDynamicDetailRows(label: string) {
    const breakdown = finalStats.bonusBreakdown;
    const sources = [
      { label: "장비 속성", stat: breakdown.gear.dynamicStats[label] },
  ...
  const extraPercentStatRows = [
    ...
    {
      key: "cryoElectricDamage",
      statKey: "cryoElectricDamage",
      label: "냉기와 전기 피해 보너스",
      value: finalStats.cryoElectricDamage,
    },
    {
      key: "heatNatureDamage",
      statKey: "heatNatureDamage",
      label: "열기와 자연 피해 보너스",
      value: finalStats.heatNatureDamage,
    },
    {
