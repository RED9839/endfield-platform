...
function resetPartyActionGauges(party: PartyMember[]) {
  return party.map((member) => ({ ...member, actionGauge: 0 }));
}
...
function reviveEnemies(enemies: BattleEnemy[]) {
  ...
  const revived = enemies.map((enemy) => {
    ...
    return { ...enemy, hp: Math.max(1, Math.ceil(enemy.maxHp * 0.35)), revived: true, statuses: withoutStatus(enemy.statuses, "defense-break") };
  });
...
function buildEnemyAction(actor: BattleEnemy, battleTurn: number) {
  ...
  return {
    ...
    gaugeDelay: (enemyHas(actor, "grab") ? 35 : 0) + (enemyHas(actor, "bind") ? 30 : 0) + (enemyHas(actor, "cold") ? 25 : 0) + (enemyHas(actor, "smoke") ? 18 : 0),
    selfDestruct,
...
function getPassiveDamageBonus(actor: PartyMember, target: BattleEnemy, kind: SkillKind) {
  let bonus = 1;
  if (actor.passiveMechanic === "essence-collapse" && target.statuses.includes("originium-crystal")) bonus += 0.2;
  if (actor.passiveMechanic === "obliteration-protocol" && target.statuses.includes("defense-break")) bonus += 0.3;
  if (actor.passiveMechanic === "blade-cut" && kind !== "attack") bonus += Math.min(5, actor.passiveStacks) * 0.08;
  return bonus;
}
...
function getLinkTarget(actor: PartyMember, battle: BattleState) {
  if (actor.linkCondition === "defense-break") return battle.enemies.find((enemy) => enemy.hp > 0 && enemy.statuses.includes("defense-break"));
  const windowTarget = battle.linkWindow?.targetEnemyId ? battle.enemies.find((enemy) => enemy.id === battle.linkWindow?.targetEnemyId && enemy.hp > 0) : undefined;
...
function canUseLinkSkill(actor: PartyMember, battle: BattleState, selectedTarget?: BattleEnemy) {
  ...
  if (actor.linkCondition === "strong-hit-vs-clean") return battle.linkWindow?.trigger === "strong-hit" && !target.statuses.includes("defense-break") && !hasArtsAttachment(target);
...
function applySkillMechanic(actor: PartyMember, target: BattleEnemy, baseDamage: number, kind: SkillKind) {
  ...
  if (actor.skillMechanic === "corrosion-support") {
    ...
    if (kind === "ultimate") notes.push("친구의 그림자");
  }
...
function applyGearStats(member: PartyMember): PartyMember {
  const stats = getEquippedGears(member.gear).reduce(
    (total, gear) => {
      ...
      return {
        ...
        speed: total.speed + (delta.speed ?? 0),
        ...
        ultimateCharge: total.ultimateCharge + (delta.ultimateCharge ?? 0),
      };
    },
    { attack: 0, hp: 0, defense: 0, evasion: 0, speed: 0, battleSkillPower: 0, linkSkillPower: 0, ultimatePower: 0, ultimateCharge: 0 },
  );
  ...
  return {
    ...
    ultimateCharge: Math.min(100, member.ultimateCharge + stats.ultimateCharge),
  };
}
...
function spendGauge<T extends { actionGauge: number }>(unit: T, amount: number): T {
  return { ...unit, actionGauge: Math.max(0, unit.actionGauge - amount) };
}
...
function tickCombatGauges(party: PartyMember[], enemies: BattleEnemy[], lockedPartyId?: string) {
  ...
  const nextParty = party.map((member) => member.hp <= 0 || member.id === lockedPartyId ? member : { ...member, actionGauge: Math.min(READY_GAUGE, member.actionGauge + member.speed * REAL_TIME_TICK) });
  const nextEnemies = enemies.map((enemy) => enemy.hp <= 0 ? enemy : { ...enemy, actionGauge: Math.min(READY_GAUGE, enemy.actionGauge + enemy.speed * REAL_TIME_TICK) });
  const timeline: TimelineEntry[] = [
    ...nextParty.filter((member) => member.hp > 0).map((member) => ({ id: member.id, name: member.name, side: "party" as UnitSide, gauge: member.actionGauge })),
    ...nextEnemies.filter((enemy) => enemy.hp > 0).map((enemy) => ({ id: enemy.id, name: enemy.name, side: "enemy" as UnitSide, gauge: enemy.actionGauge })),
  ].sort((a, b) => b.gauge - a.gauge).slice(0, 8);
  if (livingParty.length === 0 || livingEnemies.length === 0) return { party: nextParty, enemies: nextEnemies, timeline };
  const readyParty = nextParty.find((member) => member.hp > 0 && member.actionGauge >= READY_GAUGE);
  if (readyParty) return { party: nextParty, enemies: nextEnemies, activeSide: "party" as UnitSide, activeUnitId: readyParty.id, timeline };
  const readyEnemy = nextEnemies.find((enemy) => enemy.hp > 0 && enemy.actionGauge >= READY_GAUGE);
  if (readyEnemy) return { party: nextParty, enemies: nextEnemies, activeSide: "enemy" as UnitSide, activeUnitId: readyEnemy.id, timeline };
...
function resolveAutomaticBattleTurns(state: RunState) {
  ...
  for (let safety = 0; safety < 6; safety += 1) {
    ...
    const activated = { ...ticked, activeSide: ticked.activeSide ?? battle.activeSide, activeUnitId: ticked.activeUnitId ?? battle.activeUnitId };
    current = { ...current, party: activated.party, battle: { ...battle, enemies: activated.enemies, activeUnitId: activated.activeUnitId, activeSide: activated.activeSide, timeline: activated.timeline } };
    ...
    const targets = action.targetAll ? livingParty : [enemyHas(actor, "ranged") || enemyHas(actor, "sniper") ? livingParty.reduce((lowest, member) => (member.hp < lowest.hp ? member : lowest), livingParty[0]) : livingParty.reduce((lowest, member) => (member.shield < lowest.shield ? member : lowest), livingParty[0])];
    ...
    const partyAfterHit = current.party.map((member) => {
      ...
      return action.gaugeDelay > 0 ? { ...damaged, actionGauge: damaged.actionGauge - action.gaugeDelay } : damaged;
    });
    const enemiesAfterAction = activeBattle.enemies.map((enemy) => {
      if (enemy.id === actor.id) return { ...spendGauge(enemy, ENEMY_ACTION_COST), hp: action.selfDestruct ? 0 : enemy.hp };
      if (action.healsAllies && enemy.hp > 0) return { ...enemy, hp: Math.min(enemy.maxHp, enemy.hp + Math.ceil(enemy.maxHp * 0.08)) };
      return enemy;
    });
    ...
    current = { ...current, party: partyAfterHit, screen: defeated ? "summary" : "battle", result: defeated ? "defeat" : current.result, battle: { ...activeBattle, enemies: enemiesAfterAction, linkWindow: { trigger: "ally-hit" }, turn: activeBattle.turn + 1, log: [enemyLog, ...activeBattle.log].slice(0, 8) } };
...
export function useRunState(): RunState & RunActions {
  ...
  const startDeployment = useCallback(() => setState((current) => ({ ...current, screen: "map", availableNodes: current.availableNodes.length > 0 ? current.availableNodes : startingNodeIds })), []);
  ...
  const enterNode = useCallback((nodeId: string) => {
    setState((current) => {
      ...
      if (node.type === "shop") return { ...base, screen: "reward", pendingGearSlugs: chooseGearRewards(current.battlesWon + 1, 3, node.rewardTier ?? "mid") };
      ...
      return resolveAutomaticBattleTurns({ ...base, party: battleStart.party, sp: battleStart.sp, screen: "battle", battle: { enemies: getEnemies(node.enemyIds ?? []).map((enemy) => ({ ...enemy, hp: enemy.maxHp, statuses: [], actionGauge: 0 })), timeline: [], turn: 1, log: [`${node.title}에서 적과 조우했습니다.`] } });
    });
  }, []);
  ...
  const tickBattle = useCallback(() => {
    setState((current) => {
      if (!current.battle || current.screen !== "battle") return current;
      if (current.battle.activeSide === "party" && current.battle.activeUnitId) return current;
      const ticked = tickCombatGauges(current.party, current.battle.enemies, current.battle.activeSide === "party" ? current.battle.activeUnitId : undefined);
      return resolveAutomaticBattleTurns({ ...current, party: ticked.party, battle: { ...current.battle, enemies: ticked.enemies } });
    });
  }, []);
  ...
  const performAction = useCallback((operatorId: string, kind: SkillKind, targetEnemyId?: string) => {
    setState((current) => {
      ...
      const actionHitsAll = (kind === "ultimate" && actor.id !== "chenqianyu") || (kind === "link-skill" && actor.skillMechanic === "electric-attachment");
      const targetIds = new Set(actionHitsAll ? current.battle.enemies.filter((enemy) => enemy.hp > 0).map((enemy) => enemy.id) : [target.id]);
      let totalDamage = 0;
      const noteSet = new Set<string>();
      const damagedEnemies = current.battle.enemies.map((enemy) => targetIds.has(enemy.id) ? (() => {
        const mechanic = kind === "attack" ? { damage: Math.ceil(baseDamage * getStatusBonus(actor, enemy)), statuses: enemy.statuses, notes: activeSets(actor).includes("펄스식") ? ["펄스식 3세트"] : [] } : applySkillMechanic(actor, enemy, baseDamage, kind);
        ...
        const statuses = shouldInterruptEnemy(enemy, kind) ? addStatus(mechanic.statuses, "defense-break") : kind === "attack" && hp > 0 && hp <= enemy.maxHp / 2 ? addStatus(mechanic.statuses, "defense-break") : mechanic.statuses;
      ...
      const actionLabel = kind === "battle-skill" ? actor.battleSkillName : kind === "link-skill" ? actor.linkSkillName : kind === "ultimate" ? actor.ultimateName : "기본 공격";
      ...
      const linkWindow = kind === "attack" ? { trigger: "strong-hit" as const, sourceOperatorId: actor.id, targetEnemyId: target.id } : kind === "link-skill" ? { trigger: "ally-link-damage" as const, sourceOperatorId: actor.id, targetEnemyId: target.id } : undefined;
      const spAfterAction = kind === "attack" ? Math.min(current.maxSp, current.sp + 1) : kind === "battle-skill" ? current.sp - actor.battleSkillCost : actor.skillMechanic === "protective-arts" && kind === "link-skill" ? Math.min(current.maxSp, current.sp + 1) : current.sp;
      const cpAfterAction = kind === "battle-skill" ? Math.min(current.maxCp, current.cp + 1) : kind === "link-skill" ? current.cp - actor.linkSkillCost : current.cp;
      const chargeGain = kind === "attack" ? activeSets(actor).includes("펄스식") ? 20 : 14 : kind === "battle-skill" ? 24 : kind === "link-skill" ? 18 : 0;
      const partyAfterCost = current.party.map((member) => member.id === actor.id ? { ...spendGauge(member, actionCost), passiveStacks: member.passiveMechanic === "blade-cut" && kind !== "attack" ? Math.min(5, member.passiveStacks + 1) : member.passiveStacks, ultimateCharge: kind === "ultimate" ? 0 : Math.min(100, member.ultimateCharge + chargeGain) } : member);
      if (allDefeated) {
        const node = getMapNode(current.currentNodeId ?? "");
        const won = current.battlesWon + 1;
        if (node.type === "boss") return { ...current, party: partyAfterCost, sp: spAfterAction, cp: cpAfterAction, battle: { ...current.battle, enemies: enemiesAfterAction, linkWindow, log: [actionLog, ...current.battle.log] }, battlesWon: won, screen: "summary", result: "victory" };
        return { ...current, party: partyAfterCost, sp: spAfterAction, cp: cpAfterAction, battle: { ...current.battle, enemies: enemiesAfterAction, linkWindow, log: [actionLog, ...current.battle.log] }, battlesWon: won, screen: "reward", credits: current.credits + (node.type === "elite" ? 100 : 55), pendingGearSlugs: chooseGearRewards(won, 3, node.rewardTier ?? "early") };
      }
      const supportHeal = actor.skillMechanic === "corrosion-support" && kind !== "attack" ? kind === "ultimate" ? 26 : kind === "link-skill" ? 12 : noteSet.has("친구의 그림자") ? 16 : 7 : 0;
      ...
      const party = actor.skillMechanic === "protective-arts" && kind !== "attack" ? partyWithSupport.map((member) => ({ ...member, shield: member.shield + (kind === "ultimate" ? 18 : member.id === actor.id ? 14 : 8) })) : partyWithSupport;
      const defeated = party.every((member) => member.hp <= 0);
      const nextState: RunState = { ...current, party, sp: spAfterAction, cp: cpAfterAction, screen: defeated ? "summary" : "battle", result: defeated ? "defeat" : current.result, battle: { ...current.battle, enemies: enemiesAfterAction, linkWindow, turn: current.battle.turn + 1, log: [actionLog, ...current.battle.log].slice(0, 8) } };
  ...
  const equipRewardGear = useCallback((gearSlug: string, operatorId: string) => {
    setState((current) => {
      ...
      return { ...current, party: current.party.map((member) => member.id === operatorId ? equipGearToMember(member, gear) : member), collectedGears: [...current.collectedGears.filter((item) => item.slug !== replacedGear?.slug), gear], credits: current.credits + sellCredits, pendingGearSlugs: [], screen: "map", availableNodes: getMapNode(current.currentNodeId ?? "").next, battle: undefined, eventId: undefined, sp: current.maxSp };
    });
  }, []);

  const skipReward = useCallback(() => {
    setState((current) => ({ ...current, pendingGearSlugs: [], screen: "map", availableNodes: current.currentNodeId ? getMapNode(current.currentNodeId).next : startingNodeIds, battle: undefined, eventId: undefined, sp: current.maxSp }));
  }, []);
  ...
  const continueToMap = useCallback(() => {
    setState((current) => ({ ...current, screen: "map", availableNodes: current.currentNodeId ? getMapNode(current.currentNodeId).next : startingNodeIds }));
  }, []);

  return { ...state, startRun, startDeployment, abandonRun, enterNode, tickBattle, performAction, equipRewardGear, skipReward, resolveEvent, rest, continueToMap };
}
