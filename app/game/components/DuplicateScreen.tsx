  ...
  return (
    <section className="mx-auto flex min-h-[620px] w-full max-w-3xl flex-col justify-center px-4 py-10 sm:px-6">
      <div className="text-center">
        ...
        <p className="mt-3 font-mono text-[10px] font-bold uppercase tracking-[0.32em] text-ef-muted">Field Boss // 복제 보상</p>
          ...
          return (
            ...
            >
              <span className="flex items-center justify-between">
                ...
                <span className="flex items-center gap-1 font-mono text-[8px] uppercase" style={{ color: col }}>
              ...
              <span className="mt-2 line-clamp-2 font-mono text-[10px] font-bold leading-tight" style={{ color: card.effect ? "#7fd4a3" : ACCENT }}>{card.effectLine}</span>
            </button>
          );
        })}
      </div>

      <button type="button" onClick={onSkip} className="mx-auto mt-6 border border-ef-line bg-ef-card px-5 py-2.5 font-mono text-[11px] font-bold text-ef-muted transition hover:text-ef-accent-soft" style={CUT_SM}>
        복제하지 않기
      </button>
    </section>
  );
