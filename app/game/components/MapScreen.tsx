  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-start justify-between gap-3">
        ...
        <button
          type="button"
          className="flex h-12 w-12 items-center justify-center rounded-full border border-white/20 bg-white/15 text-white shadow-lg backdrop-blur transition hover:bg-white/25"
          aria-label={`${operator.name} 상세 보기`}
        >
      ...
      <div className="relative z-10 mt-2 h-[390px] overflow-hidden rounded-[28px]">
        ...
        <Image
          ...
          className="object-contain object-bottom drop-shadow-[0_28px_36px_rgba(0,0,0,0.65)] transition duration-500 group-hover:scale-105"
          priority={index === 0}
        />
      </div>
      ...
      <div className="relative z-10 -mt-2 rounded-[26px] border border-white/12 bg-black/45 p-4 shadow-[0_18px_38px_rgba(0,0,0,0.26)] backdrop-blur-xl">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="shrink-0 rounded-xl border border-cyan-200/20 bg-cyan-200/10 px-3 py-2 text-center text-xs font-black text-cyan-100">
            LV
            <span className="ml-1 text-lg text-white">{operator.level}</span>
          </span>
        </div>
...
export default function MapScreen({
  ...
  return (
    <section className="relative min-h-[calc(100vh-64px)] overflow-hidden bg-[#070914] px-4 py-6 text-white sm:px-7">
      ...
      <div className="pointer-events-none absolute left-1/2 top-16 h-96 w-96 -translate-x-1/2 rounded-full border border-cyan-200/10 bg-cyan-200/5 blur-sm" />
      ...
      <div className="relative mx-auto max-w-[1880px]">
        <header className="mb-7 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-[10px] font-black tracking-[0.42em] text-cyan-200/55">
            ...
            <p className="mt-3 text-sm font-bold text-white/45">
              목표 구역: 탈로스-II 외곽 / 위험도 A+ / 추천 Lv.60
            </p>
          </div>

          <div className="flex flex-wrap gap-3">
            <div className="rounded-full border border-cyan-200/20 bg-white/8 px-5 py-3 text-sm font-black text-cyan-100 shadow-[0_0_30px_rgba(34,211,238,0.12)] backdrop-blur-xl">
              TEAM 04 / READY
            </div>
            <div className="rounded-full border border-white/15 bg-black/25 px-5 py-3 text-sm font-black text-white/70 backdrop-blur-xl">
              POWER 128,400
            </div>
          </div>
        </header>
          ...
          <div>
            ...
            <div className="mt-6 flex flex-col gap-3 rounded-[28px] border border-white/10 bg-white/[0.055] p-4 backdrop-blur-xl md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex gap-3">
                <button type="button" className="rounded-2xl border border-white/15 bg-white/8 px-6 py-3 text-sm font-black text-white/70 transition hover:bg-white/12">
                  편성 초기화
                </button>
                <button
                  type="button"
                  onClick={() => onEnter(selectedNode.id)}
                  className="flex items-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-300 via-violet-400 to-fuchsia-500 px-7 py-3 text-sm font-black text-zinc-950 shadow-[0_0_42px_rgba(34,211,238,0.28)] transition hover:-translate-y-0.5 hover:brightness-110"
                >
                  작전 시작
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between rounded-2xl border px-4 py-3 text-left transition ${
                        active
                          ? "border-cyan-200/45 bg-cyan-200/10 text-white shadow-[0_0_28px_rgba(34,211,238,0.12)]"
