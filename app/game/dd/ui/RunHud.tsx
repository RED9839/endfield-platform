    ...
    <div className="mb-4 flex flex-wrap items-center gap-2">
      ...
      <div className="flex items-center gap-2 border border-ef-line bg-ef-card px-3 py-1.5" style={{ ...CUT, borderColor: fm.tone + "55" }}>
        ...
        <span className="flex flex-col leading-tight">
          <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-ef-muted">리전</span>
          <span className="font-mono text-sm font-bold" style={{ color: fm.tone }}>{faction} <span className="text-[10px] font-normal text-ef-muted">· {fm.region}</span></span>
        </span>
      </div>
      {/* 심층 진행도 */}
      <div className="flex items-center gap-1.5 border border-ef-line bg-ef-card px-3 py-1.5" style={CUT}>
        <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-ef-muted">심층</span>
      ...
      <div className="ml-auto flex items-center gap-2">
        <div className="flex items-center gap-1.5 border border-ef-line bg-ef-card px-2.5 py-1.5" style={CUT} title="장비 부품 — 제작·단조 재료">
          <Package className="h-3.5 w-3.5 text-ef-accent-soft" /><span className="font-mono text-sm font-bold text-white">{craft.mats.parts}</span><span className="font-mono text-[9px] text-ef-muted">부품</span>
        </div>
        <div className="flex items-center gap-1.5 border border-ef-line bg-ef-card px-2.5 py-1.5" style={CUT} title="관리권 — 제작·단조 재료">
          <KeyRound className="h-3.5 w-3.5 text-yellow-300" /><span className="font-mono text-sm font-bold text-white">{craft.mats.permits}</span><span className="font-mono text-[9px] text-ef-muted">관리권</span>
        </div>
        {onCraft && (
          <button type="button" onClick={onCraft} disabled={!canCraft} className="flex items-center gap-1.5 border border-ef-accent/50 px-3 py-1.5 font-mono text-sm font-bold uppercase tracking-wider text-ef-accent transition enabled:hover:bg-ef-accent/10 disabled:opacity-40" style={CUT}>
            <Hammer className="h-4 w-4" />제작 {owned > 0 && <span className="text-[10px] text-ef-muted">·{owned}</span>}
          </button>
        )}
      </div>
    </div>
