            ...
            return (
              <div key={e.id} className={`group relative flex w-[110px] flex-col items-center sm:w-[190px] ${shakeCls(hit, fx.tick)} ${actCls(isAct, fx.tick)}`}>
                ...
                <div className="w-full">
                  ...
                  {e.staggerMax > 0 && !dead && <div className="mt-0.5 flex items-center gap-1"><span className="inline-block w-7 shrink-0 text-[9px] leading-none text-[#a16207]/80" title="불균형: 가득 차면 행동 불가 + 받는 피해 증가">불균형</span><div className="relative flex-1"><Bar value={e.staggered ? e.staggerMax : e.stagger} max={e.staggerMax} color={e.staggered ? "#facc15" : "#a16207"} h="h-1" />{e.poiseKnot && !e.poiseBroken && <span className="pointer-events-none absolute top-[-1px] h-[calc(100%+2px)] w-px bg-white/70" style={{ left: "50%" }} title="불균형 지점 — 넘으면 잠시 중단" />}</div></div>}
