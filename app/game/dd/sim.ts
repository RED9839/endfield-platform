...
import { realAtk, skillMult, type OpProgress } from "./progress";
  ...
  const score = (sk: DDSkill) => {
    ...
    if (sk.selfUlt) {
      ...
      // (이 조건이 없어 29명 중 19명이 만충인 채 30R 내내 궁을 한 번도 안 쐈다)
        ...
        const lethal = !!t && t.hp < realAtk(self.attack) * (sk.power ?? 0) * 1.2;
...
export function enemyDrop(kind: NodeKind, depth: number, faction: string): { credits: number; parts: number; permits: number; chips: number; items: string[] } {
  ...
  const cr = k === "boss" ? 66 : k === "elite" ? 40 : 28; // 크레딧 — 0강 풀세트가 2층에 완성되게 튜닝(1층엔 미완)
  const mat = k === "boss" ? { parts: 20, permits: 4, chips: 6 } : k === "elite" ? { parts: 10, permits: 1, chips: 3 } : { parts: 0, permits: 0, chips: 0 };
  const dc = Math.floor(depth * (k === "boss" ? 5 : 2.5)); // 깊을수록 크레딧↑
