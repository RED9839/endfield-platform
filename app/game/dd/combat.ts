...
// 궁 게이지 획득 — **모든 궁충은 이 함수를 거친다**. 직접 ultCharge에 더하면 장비 부옵
// 「궁극기 충전 효율」(ultEffMul)이 조용히 새 나간다(실제로 5곳이 누락돼 있었다).
export function gainUlt(u: DDUnit, amt: number): void {
...
export type DDState = { units: DDUnit[]; round: number; log: string[]; lastLinkAlly?: string; chain?: number; skillGauge: number; maxGauge: number; boss?: boolean; anomalyConsumed?: number; allyHit?: boolean; moraleAccum?: number; forcedTargetId?: string; chaining?: boolean; linkEvents?: Record<string, number>; manualLink?: boolean; chainLinker?: string };
...
export function onAllyHit(s: DDState, self: DDUnit, t: DDUnit, final: number, log: string[]): void {
  ...
  if (self.side === "enemy") {
    ...
    if (snow) {
      ...
      applyAttach(self, "cryo", snow, log); gainUlt(snow, 10);
    }
...
export function act(s: DDState, self: DDUnit, skill: DDSkill): void {
  ...
  // 앞선 스킬 하나가 여는 연계는 1회 — 자기 연계로 자기 연계를 다시 여는 것만 막는다.
  ...
  if (self.side === "ally") {
    if (skill.kind === "battle") {
      ...
      for (const u of living(s, "ally")) {
        ...
        gainUlt(u, g); // 배율(궁충 효율·의지)은 수령자 기준으로 gainUlt가 건다
      }
    } else if (skill.kind === "link") {
      gainUlt(self, ULT_LINK); // 연계 → 시전자 +10(궁충 효율·의지 반영)
  ...
  for (const t of pickTargets(s, self, skill)) {
    ...
    if (self.id === "zhuangfangyi") { // 장방이: 청뢰검(procCount) — 연계 강제 감전 / 배틀 감전 소모 → 검 생성 + 뇌격
      if (skill.kind === "link" && t.arts.electric > 0) { // 변화의 숨결: 전기 부착 소모 → 강제 감전(이미 감전이면 레벨↑)
        ...
        gainUlt(self, 10 + 10 * n); s.anomalyConsumed = ANOMALY_WINDOW;
      ...
      if (skill.kind === "battle") { // 뇌정의 부름: 감전 소모 → 청뢰검 생성(최대 9) + 청뢰검 수 비례 뇌격(마지막 ×6) + 궁충
        ...
        if (t === primaryTarget) { // 청뢰검 생성·궁충·증폭은 1회만(변신 중 광역이어도)
          ...
          gainUlt(self, 6 * self.procCount); // 뇌격당 궁 +6
    ...
    if (self.id === "laevatain" && (skill.kind === "attack" || skill.kind === "battle" || skill.kind === "link")) {
      ...
      // 울프가드 연계(「아츠 부착 적」) 조건과 팀의 부착 빌드업이 헛돌았다.
      if (skill.kind === "attack" && (self.procCount || 0) < 4)
    ...
    if (skill.lanceRecover) { // 아비웨나 가로채기: 대상에게 누적된 썬더랜스(t.lanceN 일반 / t.lanceBig 강력) 전부 회수 → 스택 비례 중복 전기타
      ...
      if (lances + big > 0) {
        ...
        gainUlt(self, (lances + big) * 4); // 고효율 배송(회수 명중 궁 +4)
  ...
  if (self.id === "avywenna") {
    ...
    if (tg && skill.kind === "link") { tg.lanceN = (tg.lanceN || 0) + 3; gainUlt(self, 12); log.push(`  → 썬더랜스 3개 설치 → ${tg.name}(누적 일반 ${tg.lanceN}/강력 ${tg.lanceBig || 0})`); }
  ...
  if (self.id === "laevatain" && skill.kind === "link") {
    ...
    gainUlt(self, hits >= 3 ? 35 : hits === 2 ? 30 : 25);
