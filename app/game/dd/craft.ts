...
// 재료: 장비 부품 · 지역 관리권 · 전술 칩(스킬 강화 전용). 원작대로 스킬 재료는 장비 재료와 별개다.
...
// 스킬 강화 비용(현재 랭크 → +1). 9Lv(M0)→M1→M2→M3. 장비 재료가 아니라 전술 칩만 쓴다(원작: 스킬 재료 ≠ 장비 재료).
...
export const SHOP: ShopItem[] = [
  ...
  { key: "buy-chips", label: "전술 칩 ×5", kind: "mat", give: { chips: 5 }, price: 45, desc: "스킬 강화 전용 재료" },
...
// 재료 1개당 구매 단가(SHOP 번들 기준): 부품 3 · 관리권 8 · 전술칩 9
const BUY_UNIT: Record<SellMat, number> = { parts: 3, permits: 8, chips: 9 };
export const sellUnit = (mat: SellMat) => Math.max(1, Math.round(BUY_UNIT[mat] * SELL_RATE)); // 부품 1 · 관리권 2 · 전술칩 3
