...
export const forgeCost = (lv: number): CraftMats => [{ parts: 5, permits: 1 }, { parts: 9, permits: 1 }, { parts: 14, permits: 2 }][lv] ?? { parts: Infinity, permits: Infinity };
// 스킬 단조 비용(현재 랭크 → +1). 9Lv(M0)→M1→M2→M3. 스킬은 오퍼 딜 전반을 올려 장비 단조보다 비싸다.
export const skillForgeCost = (rank: number): CraftMats => [{ parts: 14, permits: 2 }, { parts: 21, permits: 3 }, { parts: 33, permits: 4 }][rank] ?? { parts: Infinity, permits: Infinity };
