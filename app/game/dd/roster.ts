...
export const SKILLS: Record<string, DDSkill[]> = {
  ...
  laevatain: [
    ...
    // 원문상 황혼은 **버프형** — 열기 부착은 궁 자체가 아니라 변신 중 「3단 평타」가 붙인다(엔진 twilight 훅에서 처리).
    // 시전 즉시 전체 열기 부착은 원문에 없는 중복이라 제거했다.
    { id: "lae-u", name: "황혼", kind: "ult", fromPos: [1, 2, 3], target: "all", power: 0, element: "heat", staggerVal: 20, selfUlt: true, note: "변신(3턴): 일반 공격 강화·광역화 + 배틀 강화 (열기 부착은 변신 중 평타가 담당)" },
  ],
...
export const ENEMY_DM: Record<string, [number, number]> = {
  ...
  "aethillu": [88347, 3465],
  "eny": [154607, 2310], // 야생의 터스크비스트 — warfarin 실측 누락(자리표시 1)로 HP 0 계산되던 버그 → 동종 안갯불 터스크비스트(154607/2310) 동급 보정
};
