...
export const SKILLS: Record<string, DDSkill[]> = {
  ...
  chenqianyu: [
    ...
    { id: "cqy-l", name: "견천하", kind: "link", fromPos: [1, 2, 3, 4], target: "row", power: 1.2, element: "physical", staggerVal: 10, cooldown: 3, anomaly: "launch", requires: (t) => !!t && t.physBreak > 0, requiresText: "방어 불능 적", note: "관통 돌진·띄우기(원문 무조건)" },
...
export function makeAlly(id: string, pos: number, progress: OpProgress = DEFAULT_PROGRESS): DDUnit {
  ...
  u.attack = Math.round((OP_ATTACK[id] ?? b.attack) * pm * skillMult(0)); // Lv90 기초 × 정예화 × M0(×1.8). 스킬 종류별 랭크 보너스는 combat이 skillRankDmg로 따로 곱한다
