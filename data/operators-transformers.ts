...
function buildLevelStats(source: SourceOperatorDetail): StatGrowth[] {
  const levels = source.levelStats.detail.levels;
  const defense = getDefaultDefense(source.class, source.rarity);
  const resistance = getDefaultResistance(source.class);

  return levels.map((level, index) => ({
    level,
    hp: source.levelStats.detail.hp[index] ?? 0,
    attack: source.levelStats.detail.atk[index] ?? 0,
    defense,
    resistance,
    power: source.levelStats.detail.str[index] ?? 0,
    agility: source.levelStats.detail.dex[index] ?? 0,
    intelligence: source.levelStats.detail.int[index] ?? 0,
    will: source.levelStats.detail.will[index] ?? 0,
  }));
}
